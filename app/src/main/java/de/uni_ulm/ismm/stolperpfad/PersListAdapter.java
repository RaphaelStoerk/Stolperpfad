...
public class PersListAdapter extends RecyclerView.Adapter<PersListAdapter.PersViewHolder> {
    ...
    private List<Person> cachedPers; // Cached copy of persons

    PersListAdapter(Context context) { mInflater = LayoutInflater.from(context); }
    ...
    public void onBindViewHolder(PersViewHolder holder, int position) {
        if (cachedPers != null) {
            Person current = cachedPers.get(position);
            holder.persItemView.setText(current.getFamilyName());
        } else {
            // Covers the case of data not being ready yet.
            holder.persItemView.setText("No Word");
        }
    }

    void setWords(List<Person> pers){
        cachedPers = pers;
